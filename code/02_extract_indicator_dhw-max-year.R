...
aggregate_raster <- function(year_i){
  ...
  for(i in 1:nrow(list_url_i)){
    ...
    if(file.exists(paste0("data/07_dhw/", list_url_i[i, "filename"])) == FALSE){
      
      # Use mode "wb" for windows otherwise issue to read the file with terra
      download.file(url = list_url_i[i, "url"],
                    destfile = paste0("data/07_dhw/", list_url_i[i, "filename"]), mode = "wb")
  ...
  ncdf_files <- list.files("data/07_dhw/", full.names = TRUE) %>% 
    as_tibble()

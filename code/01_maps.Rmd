...
# 3.1 List of EEZ to plot --

list_eez <- c("Cook Islands", "Fiji", "French Polynesia", "Gilbert Islands", "Guam", "Hawaii",
              "Howland and Baker islands", "Jarvis Island", "Johnston Atoll", "Line Group",
              "Marshall Islands", "Micronesia", "Nauru", "New Caledonia", "Niue", "Northern Mariana Islands",
              "Palau", "Palmyra Atoll", "Phoenix Group", "Pitcairn", "Samoa", "Solomon Islands",
              "Tokelau", "Tonga", "Tuvalu", "Vanuatu", "Wake Island", "Wallis and Futuna")

# 3.2 Filter and change CRS --

data_eez <- read_sf("../data/02_eez/World_EEZ_v11_20191118/eez_boundaries_v11.shp") %>% 
  filter(TERRITORY1 %in% list_eez | TERRITORY2 %in% list_eez) %>% 
...
# Test to convert to polygon

```{r}


clean_eez <- function(data, name){
  
  data_eez_a <- data %>% 
    filter(TERRITORY1 == name) %>% 
    select(-TERRITORY2)

  data_eez_b <- data %>% 
    filter(TERRITORY2 == name) %>% 
    select(-TERRITORY1) %>% 
    rename(TERRITORY1 = TERRITORY2)
  
  data_eez_i <- bind_rows(data_eez_a, data_eez_b) %>% 
    filter(LINE_TYPE != "Straight Baseline") %>%
    select(TERRITORY1) %>% 
    st_as_sf() %>% 
    st_transform(crs = 4326) %>% 
...
test <- clean_eez(data = data_eez, name = "Pitcairn") %>% 
...
ggplot() +
  geom_sf(data = test, color = "red", fill = "red")

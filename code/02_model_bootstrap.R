...
model_results <- map(1:2, ~model_bootstrap(category_i = "Hard coral",
                                           bootstrap_i = .,
                                           pdp = FALSE)) %>% 
...
model_results <- map(1:2, ~model_bootstrap(category_i = "Macroalgae",
                                           bootstrap_i = .,
                                           pdp = FALSE)) %>% 
  map_df(., ~ as.data.frame(map(.x, ~ unname(nest(.))))) %>% 
  map(., bind_rows)
...
model_results <- map(1:2, ~model_bootstrap(category_i = "Turf algae",
                                           bootstrap_i = .,
                                           pdp = FALSE)) %>% 
  map_df(., ~ as.data.frame(map(.x, ~ unname(nest(.))))) %>% 
  map(., bind_rows)
...
model_results <- map(1:2, ~model_bootstrap(category_i = "Coralline algae",
                                           bootstrap_i = .,
                                           pdp = FALSE)) %>% 
  map_df(., ~ as.data.frame(map(.x, ~ unname(nest(.))))) %>% 
  map(., bind_rows)
